...
- 学习团队合作时的 Git 使用 [掘金](https://juejin.cn/post/6976633078714204167)
  - 对`main`以及`dev*`进行了保护
...
- 完善 API 文档
- 将登陆注册的 model 层迁移到新项目中

# 07-18

- 完善 API 文档
- 与 controller 一起测试登陆注册功能并修复Bug
- 群组功能的 model&database

# 07-19

- 关于 pointer vs value
  - 切片的底层是指针，所以作为参数/返回值时直接传递切片即可，不需要使用“切片的指针”
  - 结构体视情况决定，性能上差异不大（编译器的智慧.jpg）
- GOEM 中 Update 没有查询到不会报错，需要手动判断`result.RowsAffected == 0`

# 07-20

- 添加群成员时，同步修改群组的 memberCount 信息
- 一些其他事项的开发
...
- 早上试着和前端进行联调
  ...
  - 对于没有使用 JS 框架的项目，可以使用 live sever（VSC 的插件）拉起前端服务器；或者在项目目录中运行 `python -m http.server`，启动一个前端服务器
  - 为什么要启动前端服务器：浏览器限制网页访问本地文件，可能存在跨域问题（`CORX`）。可以在`gin`路由中配置允许跨域

- 下午继续测试

  - 问题：如何标记 API 已经完成测试，避免重复测试
  - 多表联查，时灵时不灵 -> 尝试使用原生 SQL 进行建表，减少不确定性

- 进行了 code review,整理问题如下

  - dockerfile 尽量具有通用性
    - 可以在完成构建（编译）之后，将目标文件复制到新的 docker，减小镜像的体积
  - 进行失败响应
    - 可以自定义函数`func MakeFailedResponse(c *gin.context, httpStatusCode int, msg string)`
    - 不直接将`err.Error()`返回，避免敏感信息泄漏；使用统一定义的 Error；将 err 直接输出到日志
    - `c.Abort()`一般在中间件使用
  - 日志记录
    - 建立`logger`包，定义`Info`，`Error`等函数，实现形如`logger.Info(...)`的调用，减短调用链
  - 时间记录
    - 建议统一使用 UNIX 时间戳，避免时区、格式转换等等问题
  - 命名规范问题
    - JSON 一般使用 snake case, golang 中的变量/函数一般使用 camel case（根据是否导出决定是大驼峰还是小驼峰）
    - 计划：参考其他项目的命名规范（比如：database 的查询，检查类的命名）
...
- 晚上是结营仪式
  ...
  - 每个小组出一名代表发言
  - 全勤奖：茶颜的果冻
  - 围观另一个小组的 code review

- 一点小感想

  - 前期要多花时间规范设计文档，并且遵照设计文档进行开发。这样节省的是重构的时间
  - 这次最大的收获应该是第一次阅读开源项目，第一次分工（前后端分工，database 和 controller 分工）
  - 大概 15 个工作日造了:shit:*3, 有点不可思议在的
  - 最后，感谢冰岩提供的机会，感谢小伙伴的努力
